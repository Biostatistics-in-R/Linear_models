...
## Exercise: analysing the leaf mass data with an ANOVA
...
quiz(
  ...
  question(
    ...
    answer(
      "p < 0.05 indicating that at least one mean is signficantly different from at least one other", correct = TRUE
    )
  ),
   question(
...
Not all data are suitable for analysis with linear models including ANOVA. The way the analysis is calculated is based on a number of assumptions about the distribution of the data and if these are not met then the conclusions from the ANOVA can be incorrect. The important assumptions about the data for an ANOVA type model are:

1. Data points are *independent* --- in other words, each data point is a separate measure of the effect in question. In the case of the experiments we've been looking at here, one way to maximise the independence of each data point is to grow only a single plant in each pot. If they'd grown more than one plant in each pot then the measurements from the plants in a single pot would not be independent because they would be more similar to each other than to plants from other pots. This is arguably the most important assumption of all, but it is not something that can be checked after analysis. Rather, we should be making sure that our data are independent by careful study design.
...
ANOVA-type analyses, and linear models generally, are surprisingly resilient to deviations from these assumptions. They will continue to produce reliable results when the errors are somewhat non-normal or there are moderate differences in variance between groups so long as the sample size is reasonably large and the design is roughly *balanced* (meaning that the sample sizes are the same or nearly so for each group). This means that unless our sample size is very small or the design is badly unbalanced (e.g. two groups with 30 measurements and one with 3) we do not have to be especially picky and can ignore minor violations of our assumptions. Nonetheless, major violations of these assumptions will invalidate our analysis so it's important to check these  every time you run one of these analyses. We do this using what are caled *diagnostic plots*. This video explains what these plots are showing and what to look out for.
...
```{r fig.cap = "Diagnostic plots for the root mass ANOVA"}
...
## Exercise: diagnostics for the leaf mass ANOVA

The first thing to do is to plot our two diagnostic plots for your fitted ANOVA on leaf mass.

```{r diagnostics, exercise = TRUE, exercise.cap = "Diagnostics", exercise.lines = 5, exercise.setup = "prepare-anova" }
